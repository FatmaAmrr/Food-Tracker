...
def index():
    ...
    cur = db.execute('''select log_date.entry_date, sum(food.protein) as protein, sum(food.carbohydrates) 
                     as carbohydrates, sum(food.fat) as fat, sum(food.calories) as calories from log_date 
    ...
    for i in results:
        ...
        single_date['good_date'] = datetime.strftime(d, '%B %d, %Y')
...
def view(date):
    ...
    good_date = datetime.strftime(d, '%B %d, %Y')
    ...
    return render_template('day.html', entry_date=date_result['entry_date'], good_date=good_date,
...
def food():
    ...
    if request.method == 'POST':
        ...
        # return '<h1>Name: {} Protein: {} Carbs: {} Fat: {}</h1>'.format(request.form['food-name'], request.form[
        # 'protein'], request.form['carbohydrates'], request.form['fat'])
